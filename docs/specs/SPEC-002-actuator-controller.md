...
- 限流電阻串聯每顆 LED；阻值依 LED 順向電壓 Vf：
  - 綠/黃/紅（Vf≈2.0V）：**220Ω** → ~6mA
  - 藍/白（Vf≈3.0V）：**100Ω** → ~3mA（220Ω 會過暗）
- GPIO 腳位最大額定：40mA → 全亮總電流遠低於上限，安全
- 依據：AI-course `hardware/wiring.md` 實測接線經驗（見 ADR-002 釐清）

> 注意：此為軟體 SPEC，電阻為硬體接線參數；實際阻值以實機點燈亮度為準。
